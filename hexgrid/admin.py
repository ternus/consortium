...
class ItemAdmin(admin.ModelAdmin):
    def formfield_for_foreignkey(self, db_field, request=None, **kwargs):
        if db_field.name == 'gto':
            kwargs['queryset'] = GameTeXObject.objects.filter(macro=GameTeXFieldValue.objects.filter(field__name=PRICE_FIELD).exclude(value=u'').values_list('object', flat=True))
